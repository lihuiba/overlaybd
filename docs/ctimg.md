...
- **Filesystem-based images.** The dominant OCI tar+gzip format is
  ...
  secure containers — the filesystem must additionally be shared into
  the guest through virtio-fs or 9p, so every filesystem-based stack
  ends up depending on overlayfs-style stacking, a FUSE/virtio-fs
  serving path, or both.
...
FUSE/fscache, filesystem state lives in a userspace daemon; if it
...
- **O(1) cross-layer lookup.** Because the layer indices are merged at
  ...
  hundreds of millions of QPS for metadata resolution.
